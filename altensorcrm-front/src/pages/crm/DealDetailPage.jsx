...
const DealDetailPage = () => {
  ...
  const [dealComments, setDealComments] = useState(() => {
    if (typeof window !== 'undefined') {
      try {
        ...
        const cached = localStorage.getItem(`crm_deal_comments_${urlId}`);
  ...
  const findOrCreateDealTask = async (dealId) => {
    try {
      const tasks = await taskManagementApi.getAllTasks();
      const list = Array.isArray(tasks) ? tasks : (tasks?.items || tasks?.data || []);
      let dealTask = list.find(t =>
      ...
      if (!dealTask) {
        ...
        dealTask = await taskManagementApi.createTask({
          title: `Deal Activity Task #${dealId}`,
          description: `Activity and Comments for Deal #${dealId} [DEAL_ID:${dealId}]`,
          difficulty: 1,
          status: 0,
          createdByUserId: currentUser?.userId || currentUser?.id || ''
        });
      }
      return dealTask?.id || dealTask?.Id;
    } catch (err) {
      console.warn('Error resolving deal task:', err);
      return null;
    }
  };
  ...
  useEffect(() => {
    const loadTaskCommentsAndUsers = async () => {
      try {
        ...
        if (id) {
          const targetTaskId = await findOrCreateDealTask(id);
          if (targetTaskId) {
            setAssociatedTaskId(targetTaskId);
            const taskDetail = await taskManagementApi.getTaskById(targetTaskId);
            if (taskDetail && taskDetail.taskComments && Array.isArray(taskDetail.taskComments)) {
              setDealComments(taskDetail.taskComments);
              localStorage.setItem(`crm_deal_comments_${id}`, JSON.stringify(taskDetail.taskComments));
            }
          }
        }
      } catch (err) {
  ...
  const handlePostTaskComment = async (e) => {
    ...
    try {
      ...
      if (!targetId && id) {
        targetId = await findOrCreateDealTask(id);
      ...
      if (targetId) {
        ...
        try {
          const taskDetail = await taskManagementApi.getTaskById(targetId);
          if (taskDetail && taskDetail.taskComments && Array.isArray(taskDetail.taskComments)) {
            setDealComments(taskDetail.taskComments);
            localStorage.setItem(`crm_deal_comments_${id}`, JSON.stringify(taskDetail.taskComments));
          } else {
            throw new Error('Manual push');
          }
        } catch {
          const currentUser = getCurrentUser();
          const newCommentObj = {
            ...
            user: { userName: currentUser?.userName || currentUser?.name || 'Administrator', avatarUrl: currentUser?.avatarUrl },
            createAt: new Date().toISOString()
          };
          const updated = [newCommentObj, ...dealComments];
          setDealComments(updated);
          localStorage.setItem(`crm_deal_comments_${id}`, JSON.stringify(updated));
        }
      }
      setNewCommentInput('');
  ...
  const fetchDealAttachments = async (dealId) => {
    if (!dealId) return;
    try {
      const tasks = await taskManagementApi.getAllTasks();
      const list = Array.isArray(tasks) ? tasks : (tasks?.items || tasks?.data || []);
      const dId = String(dealId).toLowerCase();
      const matchedTasks = list.filter((t) => {
        const title = (t.title || '').toLowerCase();
        const desc = (t.description || '').toLowerCase();
        return title.includes(`deal #${dId}`) ||
          title.includes(`deal activity task #${dId}`) ||
          desc.includes(`[deal_id:${dId}]`) ||
          desc.includes(`deal #${dId}`);
      });
  ...
  const activitiesTimeline = useMemo(() => {
    ...
    // 5. Deal Creation Event
    const creationDate = formData.closedDate || formData.createdAt || new Date().toISOString();
    items.push({
      id: 'deal_created',
      type: 'created',
      author: formData.dealOwner || 'Administrator',
      initial: (formData.dealOwner || 'A').charAt(0).toUpperCase(),
      title: language === 'az' ? 'bu sövdələşməni yaratdı' : language === 'en' ? 'created this deal' : 'создал эту сделку',
    ...
    return items;
  }, [dealComments, notesList, callLogsList, attachmentsList, formData, language]);
  ...
  useEffect(() => {
    const initPage = async () => {
      ...
      if (id) {
        await fetchDealDetail(id, users);
        await fetchDealNotesAndCalls(id);
        await fetchDealAttachments(id);
      }
    };
    initPage();
  }, [id]);

  const fetchDealNotesAndCalls = async (dealId) => {
    try {
      ...
      if (Array.isArray(allNotes)) {
        const dNotes = allNotes.filter(n => String(n.dealId || n.DealId) === String(dealId || id));
        setNotesList(dNotes.length > 0 ? dNotes : allNotes.filter(n => !n.leadId && !n.LeadId));
      }
      if (Array.isArray(allCalls)) {
        const dCalls = allCalls.filter(c => String(c.dealId || c.DealId) === String(dealId || id));
        setCallLogsList(dCalls.length > 0 ? dCalls : allCalls.filter(c => !c.leadId && !c.LeadId));
      }
    } catch (err) {
  ...
  const fetchDealDetail = async (dealId, currentOwners = null) => {
    try {
      setLoading(true);
      const data = await dealsApi.getById(dealId);
      if (data) {
        const activeOwners = currentOwners || ownerList;
        const ownerObj = activeOwners.find(o => String(o.id) === String(data.dealOwnerId || data.assignedToId || data.ownerId || data.userId)) ||
                         activeOwners.find(o => o.name === (data.dealOwnerName || data.assignedTo || data.ownerName || data.owner));
        const resolvedOwner = data.dealOwnerName || data.dealOwner?.name || ownerObj?.name || data.assignedTo || 'Administrator';
  ...
  const handleSave = async (overrideData = null) => {
    ...
    try {
      setSaving(true);
      const matchedOwner = ownerList.find(o => o.name === dataToSave.dealOwner);
      ...
      const payload = {
        ...
        dealOwnerId: matchedOwner?.id || null,
        organizationId: null,
        contactId: null
      };
  ...
  return (
    <div className="-m-4 lg:-m-6 -mb-20 min-h-screen bg-[#121214] text-[#E4E4E7] flex flex-col font-sans relative">
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between border-b border-[#2C2C2E]/40 pb-3.5">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      ...
                      <span>{language === 'az' ? 'Şərh yaz' : language === 'en' ? 'Add Comment' : 'Комментарий'}</span>
                    </button>
                    <button
                      type="button"
                      ...
                      className="flex items-center gap-1.5 bg-[#1C1C1E] border border-[#2C2C2E] hover:border-[#3F3F46] px-3 py-1.5 rounded-xl text-xs font-semibold text-white transition-colors cursor-pointer"
                    >
                      <DocumentTextIcon className="w-3.5 h-3.5 text-amber-400" />
                      <span>{language === 'az' ? 'Qeyd yaz' : language === 'en' ? 'Add Note' : 'Заметка'}</span>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <span>{language === 'az' ? 'Fayl əlavə et' : language === 'en' ? 'Attach File' : 'Прикрепить файл'}</span>
                    </button>
                  </div>
                </div>
                ...
                {activitiesTimeline.length === 0 ? (
                  <div className="py-16 text-center text-[#71717A] text-xs">
                    {language === 'az' ? 'Hələ ki heç bir fəaliyyət qeydə alınmayıb.' : language === 'en' ? 'No activities recorded yet.' : 'Активностей пока нет.'}
                  </div>
                ) : (
                  <div className="space-y-6 relative pl-6 pr-1">
                                        ...
                                        {filteredProds.length === 0 ? (
                                          ...
                                          filteredProds.map((prod) => (
                                            <button
                                              ...
                                              className={`w-full text-left px-2.5 py-1.5 rounded-xl transition-colors cursor-pointer flex items-center justify-between ${p.productId === prod.id ? 'bg-[#2C2C2E] text-white font-semibold' : 'hover:bg-[#2C2C2E]/60 text-[#D4D4D8]'
                                                }`}
                                            >
      ...
      {isNewProductModalOpen && (
        <div 
          className="fixed inset-0 z-[150] bg-black/80 backdrop-blur-xs flex items-center justify-center p-2 sm:p-4 overflow-y-auto"
          onClick={(e) => { if (e.target === e.currentTarget) { setIsNewProductModalOpen(false); setEditingProductId(null); } }}
        >
          <div 
            className="bg-[#1C1C1E] border border-[#2C2C2E] rounded-2xl sm:rounded-3xl shadow-2xl w-full max-w-xl text-[#E4E4E7] flex flex-col max-h-[92vh] sm:max-h-[85vh] overflow-hidden my-auto animate-in fade-in duration-200"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <form onSubmit={handleCreateProductSubmit} className="flex flex-col flex-1 min-h-0">
              <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 text-xs custom-scrollbar">
      ...
      {isAttachModalOpen && (
        <div 
          ...
          onClick={(e) => { if (e.target === e.currentTarget) { setIsAttachModalOpen(false); setSelectedAttachFile(null); setAttachLinkUrl(''); } }}
        >
          <div 
            className="bg-[#1C1C1E] border border-[#2C2C2E] rounded-2xl sm:rounded-3xl shadow-2xl w-full max-w-lg p-4 sm:p-6 text-[#E4E4E7] space-y-6 animate-in fade-in duration-200 my-auto max-h-[92vh] sm:max-h-[85vh] overflow-y-auto custom-scrollbar"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Modal Header */}
            <div className="flex items-center justify-between border-b border-[#2C2C2E]/60 pb-3">
              <h2 className="text-base sm:text-lg font-bold text-white tracking-tight">{language === 'az' ? 'Əlavə et' : language === 'en' ? 'Attach' : 'Прикрепить'}</h2>
              <button
                type="button"
                onClick={() => {
                  setIsAttachModalOpen(false);
              ...
              <div className="flex items-center justify-center gap-6">
                ...
                <div className="flex flex-col items-center gap-1.5">
                  <label
                    ...
                    className={`p-3 rounded-2xl border transition-all cursor-pointer flex flex-col items-center justify-center ${attachTab === 'device' ? 'bg-[#2C2C2E] border-sky-500 text-sky-400' : 'bg-[#1C1C1E] border-[#2C2C2E] text-white hover:bg-[#2C2C2E]'
                      }`}
                ...
                <div className="flex flex-col items-center gap-1.5">
                  <button
                    ...
                    className={`p-3 rounded-2xl border transition-all cursor-pointer flex flex-col items-center justify-center ${attachTab === 'link' ? 'bg-[#2C2C2E] border-sky-500 text-sky-400' : 'bg-[#1C1C1E] border-[#2C2C2E] text-white hover:bg-[#2C2C2E]'
                      }`}
                  >
                ...
                <div className="flex flex-col items-center gap-1.5">
                  <label
                    ...
                    className={`p-3 rounded-2xl border transition-all cursor-pointer flex flex-col items-center justify-center ${attachTab === 'camera' ? 'bg-[#2C2C2E] border-sky-500 text-sky-400' : 'bg-[#1C1C1E] border-[#2C2C2E] text-white hover:bg-[#2C2C2E]'
                      }`}
            ...
            <div className="flex items-center justify-end pt-2">
              <button
                ...
                className={`px-5 py-2 rounded-xl text-xs font-semibold transition-colors shadow-sm ${(selectedAttachFile || attachLinkUrl)
