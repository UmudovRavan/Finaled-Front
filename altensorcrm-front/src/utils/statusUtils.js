export const normalizeLeadStatus = (status) => {
  ...
  if (s === '5' || low === 'converttodeal' || low === 'converted' || low === 'çevrildi' || low === 'конвертирован') return 'Converted';
  if (s === '1' || low === 'connected' || low === 'contacted' || low === 'əlaqə saxlanıldı' || low === 'elaqe saxlanildi' || low === 'связались') return 'Contacted';
  if (s === '0' || low === 'new' || low === 'draft' || low === 'yeni' || low === 'новый') return 'New';
  if (s === '2' || low === 'nurture' || low === 'i̇nkişaf etdirilən' || low === 'inkisaf etdirilen' || low === 'развиваемый') return 'Nurture';
  if (s === '3' || low === 'qualified' || low === 'uyğun' || low === 'uygun' || low === 'квалифицирован') return 'Qualified';
  if (s === '4' || low === 'unqualified' || low === 'uyğun olmayan' || low === 'uygun olmayan' || low === 'не квалифицирован') return 'Unqualified';
  if (s === '6' || low === 'junk' || low === 'yararsız' || low === 'yararsiz' || low === 'спам') return 'Junk';
  if (s === '7' || low === 'lost' || low === 'itirildi' || low === 'утерян') return 'Lost';
  return s;
};
