...
const ProtectedRoute = ({ children, requiredModule = 'CRM', requiredPermission }) => {
  const { user, isAuthenticated, authChecked, hasModule, hasPermission, hasRole, logout } = useAuth();
  ...
  const isAdmin = hasRole && (hasRole('TenantAdmin') || hasRole('PlatformSuperAdmin') || hasRole('Admin') || hasRole('SuperAdmin'));
  if (requiredPermission && !isAdmin && (!hasPermission || !hasPermission(requiredPermission))) {
    return (
      <div
        style={{
          minHeight: '100vh',
      ...
      >
        ...
        >
          ...
          <p style={{ fontSize: '14px', color: '#94A3B8', marginBottom: '24px', lineHeight: '1.6' }}>
            Bu CRM bölməsinə daxil olmaq üçün tələb olunan icazəniz (<span style={{ color: '#F43F5E', fontWeight: 600 }}>{requiredPermission}</span>) yoxdur. Zəhmət olmasa platform administratoru ilə əlaqə saxlayın.
          </p>
          <button
            onClick={() => {
